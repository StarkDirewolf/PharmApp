...
namespace PharmApp.src.CV.Screens
{
    class ScreenOrderPad : ScreenProScript
    {
        ...
        public override List<OCRResult> GetPipcodes(Image<Bgr, byte> screen)
        {
            ...
            OCRResult orderCode = OCR.Get().FindFirstText(screen, "Order Code", new Rectangle(ORDERCOLUMNS_X, ORDERCOLUMNS_Y, ORDERCOLUMNS_WIDTH, ORDERCOLUMNS_HEIGHT),
            ...
            if (orderCode != null)
            {
                ...
                Rectangle orderCodeRect = orderCode.GetRectangle();
                searchArea = new Rectangle(orderCodeRect.X, orderCodeRect.Y + orderCodeRect.Height, PIPCODES_COLUMN_WIDTH, PIPCODES_COLUMN_HEIGHT);
